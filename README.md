...
## 🖥️ Mask Validation GUI

AutoSlide includes a powerful GUI tool for validating masks generated by the pipeline:

### Features
- **Side-by-side visualization** - View original images, masks, and overlays simultaneously
- **Keyboard navigation** - Efficient browsing with arrow keys and shortcuts
- **Quality control** - Mark problematic masks or entire image sets for exclusion
- **Real-time overlay generation** - Create overlays on-the-fly when not available
- **Mask binarization** - Toggle between grayscale and binary mask views with adjustable thresholds
- **Autosave functionality** - Automatically saves validation results every 30 seconds
- **Batch validation** - Process entire directories of pipeline outputs

### Usage
```bash
# Launch GUI with directory selection dialog
python src/utils/mask_validation_gui.py

# Launch GUI with specific directory
python src/utils/mask_validation_gui.py /path/to/pipeline/output
```

### Keyboard Shortcuts
- **↑/↓** - Navigate between images
- **←** or **M** - Drop/restore current mask
- **→** or **A** - Drop/restore entire image set
- **B** - Toggle mask binarization
- **O** - Toggle force recreate overlay
- **G** - Focus jump-to-image textbox
- **S** - Save validation results
- **Q** - Quit application

### Output
The GUI saves validation results to `validation_results.json` containing:
- List of dropped masks and images
- Summary statistics
- Validation metadata for downstream processing
