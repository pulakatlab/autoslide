...
def quantify_fibrosis(
    ...
    if vessel_mask is not None:
        ...
        fibrosis_percentage = (fibrosis_area_no_vessels / tissue_area) * 100 if tissue_area > 0 else 0
...
def find_images_with_masks(data_dir, verbose=False):
    ...
    for image_path in image_paths:
        ...
        if os.path.exists(mask_path):
            ...
            hash_value = image_name.split('_')[-1].replace('.png', '') if '_' in image_name else None
    ...
    if verbose:
        ...
        if images_with_masks:
            ...
            for i, item in enumerate(images_with_masks[:3]):
                print(f"  {i+1}. {item['image_name']} (type: {item['region_type']})")
...
def create_fibrosis_visualization(image, fibrosis_mask, vessel_mask, results, image_name, vis_dir, verbose=False):
    ...
    try:
        ...
        vessel_binary = (vessel_mask > 127).astype(bool) if vessel_mask.max() > 1 else vessel_mask.astype(bool)
...
def create_summary_statistics(results_df, results_dir, verbose=False):
    ...
    try:
        ...
        if verbose:
            ...
            print(f"Mean fibrosis percentage: {summary_stats['mean_fibrosis_percentage']:.2f}%")
        
        # Create summary plots
        fig, axes = plt.subplots(2, 2, figsize=(12, 10))
        
        # Histogram of fibrosis percentages
        axes[0, 0].hist(results_df['fibrosis_percentage'], bins=20, alpha=0.7, edgecolor='black')
        axes[0, 0].set_xlabel('Fibrosis Percentage (%)')
        axes[0, 0].set_ylabel('Frequency')
        axes[0, 0].set_title('Distribution of Fibrosis Percentages')
        axes[0, 0].axvline(summary_stats['mean_fibrosis_percentage'], color='red', linestyle='--', label='Mean')
        axes[0, 0].legend()
        ...
        if 'region_type' in results_df.columns and results_df['region_type'].nunique() > 1:
            ...
            region_labels = [rt for rt in results_df['region_type'].unique() if pd.notna(rt)]
        ...
        axes[1, 0].scatter(results_df['total_area'], results_df['fibrosis_area'], alpha=0.6)
...
def parse_args():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Fibrosis quantification on images with neural network masks')
