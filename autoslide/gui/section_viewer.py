    ...
    def on_filter_change(self, *args):
        ...
        if not filter_text:
            ...
            if filter_col == "All":
                ...
                mask = self.data[filter_col].astype(
                    str).str.lower().str.contains(filter_text, na=False)
...
class SectionViewer:
    ...
    def setup_menu(self):
        ...
        view_menu.add_command(label="Refresh Images",
    ...
    def setup_dataframe_panel(self, parent):
        ...
        ttk.Label(title_frame, text="Sections Database",
    ...
    def setup_image_panels(self, parent):
        ...
        section_frame.pack(side=tk.LEFT, fill=tk.BOTH,
                           expand=True, padx=5, pady=5)

        self.section_canvas = tk.Canvas(
            section_frame, bg='white', width=400, height=300)
        ...
        self.section_info_text = tk.Text(
            info_frame, height=8, width=30, wrap=tk.WORD)
        info_scroll = ttk.Scrollbar(
            info_frame, orient=tk.VERTICAL, command=self.section_info_text.yview)
        ...
        downsample_spinbox.bind(
            '<Return>', lambda e: self.on_downsample_change())
        ...
        self.slide_canvas = tk.Canvas(
            bottom_frame, bg='lightgray', width=800, height=300)
        ...
        try:
            ...
            if not os.path.exists(tracking_dir):
                messagebox.showerror(
                    "Error", f"Tracking directory not found: {tracking_dir}")
                return
            ...
            self.sections_df = load_suggested_regions(
            ...
            messagebox.showinfo(
                "Success", f"Loaded {len(self.sections_df)} sections")
    ...
    def update_section_info(self):
        ...
        self.mask_validated_var.set(
            bool(self.current_section.get('mask_validated', False)))
        self.include_image_var.set(
            bool(self.current_section.get('include_image', True)))
    ...
    def load_section_image(self):
        ...
        try:
            ...
            section, section_details = get_section_from_hash(
                section_hash, self.sections_df, down_sample=downsample_factor)
    ...
    def find_prediction_mask(self, section_hash):
        ...
        mask_pattern = os.path.join(
            suggested_regions_dir, '**', 'masks', f'*{section_hash}_mask.png')
    ...
    def display_section_image(self):
        ...
        if self.section_mask is not None:
            pil_image = self.add_prediction_overlay(
                pil_image, self.section_mask)
    ...
    def add_prediction_overlay(self, image, mask):
        ...
        if len(contours) > 0:
            cv2.drawContours(img_array, contours, -1,
                             (255, 255, 0), 2)  # Yellow contours
    ...
    def display_slide_overview(self, slide_sections):
        ...
        try:
            ...
            slide_image = self.current_slide_scene.read_block(
                size=(slide_rect[2] // down_sample,
                      slide_rect[3] // down_sample)
            )
                ...
                try:
                    ...
                    if section['section_hash'] == current_hash:
                        ...
                        draw.rectangle([x1, y1, x2, y2], fill=(
                            255, 0, 0, 128), outline=(255, 0, 0), width=3)
                    else:
                    ...
                    print(
                        f"Error drawing section {section['section_hash']}: {e}")
                    continue
    ...
    def on_property_change(self):
        ...
        self.sections_df.loc[mask,
                             'mask_validated'] = self.mask_validated_var.get()
        self.sections_df.loc[mask,
            ...
            for basename, group in self.sections_df.groupby('basename'):
                ...
                if csv_files:
                    ...
                    columns_to_save = list(
                        original_df.columns) + ['mask_validated', 'include_image']

                    # Ensure we only save columns that exist
                    available_columns = [
                        col for col in columns_to_save if col in group.columns]
    ...
    def open_section_folder(self):
        ...
        try:
            ...
            suggested_regions_dir = os.path.join(
                config['data_dir'], 'suggested_regions')
            section_folders = glob(os.path.join(
                suggested_regions_dir, f"*{basename}*"))
    ...
    def view_full_size(self):
        ...
        full_size_window.title(
            f"Full Size - {self.current_section['section_hash']}")
        ...
        try:
            section_hash = self.current_section['section_hash']
            full_section, _ = get_section_from_hash(
                section_hash, self.sections_df, down_sample=1)
