...
class EditableDataFrame:
    ...
    def setup_ui(self):
        ...
        self.tree = ttk.Treeview(
            tree_frame, columns=display_columns, show='headings', height=15)

        # Configure columns
        column_titles = {'label': 'Initial Label',
                         'tissue_type': 'Tissue Type', 'tissue_num': 'Tissue Number'}
        ...
        self.edit_entry = ttk.Entry(
            edit_frame, textvariable=self.edit_var, width=20)
        self.edit_entry.pack(side=tk.LEFT, padx=5)
        self.edit_entry.bind('<Return>', self.apply_edit)

        ttk.Button(edit_frame, text="Apply", command=self.apply_edit).pack(
            side=tk.LEFT, padx=5)
    ...
    def apply_edit(self, event=None):
        ...
        try:
            if self.current_edit_column == 'tissue_num':
                ...
                ) if new_value.strip() else np.nan
            ...
            messagebox.showerror(
                "Invalid Value", f"Invalid value for {self.current_edit_column}: {e}")
    ...
    def quick_tissue_type(self, tissue_type):
        ...
        if pd.isna(self.data.loc[row_idx, 'tissue_num']):
            ...
            existing_nums = self.data['tissue_num'].dropna().astype(
                int).tolist()
            next_num = 1
...
class AnnotationGUI:
    ...
    def setup_menu(self):
        ...
        file_menu.add_command(label="Process Missing Initial Annotations",
                              command=self.process_missing_annotations)
        file_menu.add_command(label="Open Data Directory",
        ...
        settings_menu.add_command(
            label="Annotation Parameters", command=self.show_parameters_dialog)
    ...
    def setup_status_bar(self):
        ...
        self.status_label = ttk.Label(
            self.status_frame, textvariable=self.status_var, relief=tk.SUNKEN)
    ...
    def setup_editing_tab(self):
        ...
        edit_file_frame = ttk.LabelFrame(
            editing_frame, text="Select Processed File")
        ...
        self.processed_files_listbox = tk.Listbox(
            list_container, height=8, selectmode=tk.SINGLE)
        files_scrollbar = ttk.Scrollbar(
            list_container, orient=tk.VERTICAL, command=self.processed_files_listbox.yview)
        self.processed_files_listbox.configure(
            yscrollcommand=files_scrollbar.set)

        self.processed_files_listbox.pack(
            side=tk.LEFT, fill=tk.BOTH, expand=True)
        files_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        # Bind single-click to load file automatically
        self.processed_files_listbox.bind(
            '<<ListboxSelect>>', lambda e: self.load_for_editing())
    ...
    def check_and_process_initial_annotations(self):
        ...
        slide_files = glob(os.path.join(
            data_dir, '**', glob_pattern), recursive=True)
    ...
    def refresh_processed_files(self):
        ...
        if os.path.exists(annot_dir):
            ...
            basenames = [os.path.basename(f).replace(
                '.csv', '') for f in csv_files]
    ...
    def process_single_file(self, file_path, show_preview=True):
        ...
        try:
            ...
            # Get threshold mask
            threshold_mask = get_threshold_mask(
                scene, down_sample=self.down_sample)

            # Dilate mask
            dilation_kern = np.ones(
                (self.dilation_kern_size, self.dilation_kern_size))
    ...
    def load_for_editing(self):
        ...
        if not selection:
            messagebox.showwarning(
                "No Selection", "Please select a processed file")
            return
        ...
        try:
            ...
            if not os.path.exists(csv_path):
                messagebox.showerror(
                    "File Not Found", f"CSV file not found: {csv_path}")
                return
    ...
    def update_edit_preview(self):
        ...
        if portrait_raw_image is not None:
            ...
            self.edit_axes[0].set_title(
                'Raw Slide Image (Not Available)', fontsize=14, weight='bold')
        ...
        for _, row in self.current_metadata.iterrows():
            ...
            if was_rotated:
                ...
                adjusted_centroid = (
                    centroid[1], original_height - centroid[0])
            else:
    ...
    def auto_finalize_annotation(self):
        ...
        try:
            ...
            original_mask_path = os.path.join(
                fin_annotation_dir, selected_file + '_original.npy')
            ...
            for _, row in self.current_metadata.iterrows():
                ...
                if not pd.isna(row['tissue_num']):
                    ...
                    final_mask[self.current_mask ==
                               original_label] = original_label + 1000

            # Save final mask
            final_mask_path = os.path.join(
                fin_annotation_dir, selected_file + '.npy')
            ...
            image_label_overlay = label2rgb(
                final_mask, image=final_mask > 0, bg_label=0)

            fig, ax = plt.subplots(figsize=(10, 8))
            ax.imshow(image_label_overlay)
            ax.set_title(
                f'{selected_file} - Final Annotation (Colored by Tissue Number)')
            ...
            for _, row in self.current_metadata.iterrows():
                ...
                final_mask_rotated = final_mask.shape[1] > final_mask.shape[
                    0] and self.current_mask.shape[0] > self.current_mask.shape[1]
    ...
    def show_parameters_dialog(self):
        ...
        # Down sample
        ttk.Label(params_frame, text="Down Sample Factor:").grid(
            row=0, column=0, sticky=tk.W, padx=5, pady=5)
        down_sample_var = tk.IntVar(value=self.down_sample)
        ttk.Spinbox(params_frame, from_=10, to=500, textvariable=down_sample_var,
                    width=10).grid(row=0, column=1, padx=5, pady=5)

        # Dilation kernel size
        ttk.Label(params_frame, text="Dilation Kernel Size:").grid(
            row=1, column=0, sticky=tk.W, padx=5, pady=5)
        dilation_var = tk.IntVar(value=self.dilation_kern_size)
        ttk.Spinbox(params_frame, from_=1, to=10, textvariable=dilation_var,
                    width=10).grid(row=1, column=1, padx=5, pady=5)

        # Area threshold
        ttk.Label(params_frame, text="Area Threshold:").grid(
            row=2, column=0, sticky=tk.W, padx=5, pady=5)
        area_var = tk.IntVar(value=self.area_threshold)
        ttk.Spinbox(params_frame, from_=1000, to=100000, textvariable=area_var,
        ...
        ttk.Button(button_frame, text="Apply", command=apply_params).pack(
