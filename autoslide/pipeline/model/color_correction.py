...
class ColorCorrector:
    ...
    def _compute_reference_statistics(self):
        ...
        logger.info(f"Computed reference statistics from {len(self.reference_images)} images")
...
def compute_color_divergence(
    ...
    if metric == 'euclidean':
        ...
        for i in range(3):
            ...
            kl_div += (std1[i]**2 + (mean1[i] - mean2[i])**2) / (2 * std2[i]**2)
            kl_div -= 0.5
        return kl_div
...
def batch_color_correction(
    ...
    logger.info(f"Batch correction complete: {successful} successful, {failed} failed")
