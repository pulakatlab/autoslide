...
def mock_slide_scene():
    ...
    mock_scene.read_block.return_value = np.random.randint(0, 255, (100, 100, 3), dtype=np.uint8)
...
class TestInitialAnnotation:
    ...
    def test_initial_annotation_workflow(self, mock_config, mock_get_threshold_mask, 
        ...
        # Setup mocks
        mock_config.__getitem__.side_effect = lambda key: temp_dirs['data_dir'] if key == 'data_dir' else None
        ...
        with patch('autoslide.pipeline.annotation.initial_annotation.glob') as mock_glob:
            ...
            assert os.path.exists(os.path.join(temp_dirs['data_dir'], 'initial_annotation'))
            assert os.path.exists(os.path.join(temp_dirs['data_dir'], '.tracking'))
...
class TestFinalAnnotation:
    ...
    def test_label_mapping(self, sample_mask, sample_metadata_df):
        ...
        np.testing.assert_array_equal(sorted(unique_values), sorted(expected_values))
    ...
    def test_json_processing(self, mock_glob, mock_config, temp_dirs, sample_tracking_json):
        ...
        sample_metadata_df().to_csv(json_data['wanted_regions_frame_path'], index=False)
...
class TestGetSectionDetails:
    ...
    def test_section_metadata_loading(self, mock_config, mock_open_slide, temp_dirs, mock_slide_scene):
        ...
        section_dir = os.path.join(temp_dirs['data_dir'], 'suggested_regions', test_basename)
        ...
        test_svs_path = os.path.join(temp_dirs['data_dir'], f'{test_basename}.svs')
        with open(test_svs_path, 'w') as f:
            f.write('dummy')
    ...
    def test_image_comparison(self, mock_imread):
        ...
        extracted_img = np.random.randint(0, 255, (100, 100, 3), dtype=np.uint8)
...
class TestIntegration:
    ...
    def test_annotation_pipeline_flow(self, temp_dirs, sample_metadata_df, sample_mask):
        ...
        tracking_json_path = os.path.join(temp_dirs['tracking_dir'], 'test.json')
