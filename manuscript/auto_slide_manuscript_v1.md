...
Annotation of histological images is a crucial task for many biomedical analyses. However, variability and complexity of structural features in histological images poses significant challenges for quantitative analyses due to factors such as significant variability of features within a tissue, variability of staining across batches, and subjectivity of labelling and annotation across users. In much cardiac research, measurement of interstitial fibrosis is a crucial step in determining structural health of cardiac tissue; however, in addition to the above confounds, contamination from perivascular collagen significantly affectsconfounds this calculation, requiring removal of blood vessels prior to measurement. The conventional approach of manual annotation is, therefore, 1\) labor-intensive, 2\) biased by user-specificity (not reproducible (due to user idiosyncrasiesbiased by user-specificity (), and 3\) brittle (due to the need for manual tuning), making it simply unscalable. Machine learning-based pipelines hold promise in providing a robust and reproducible solution to this issue, and while there is active research on use of such pipelines for measurement of renal pathology and detection of cancerous tissue, littleno work has been done on applying these models to cardiac assessment. Here, we present an end-to-end pipeline for quantification of interstitial and perivascular fibrosis in Mason’s Trichrome-stained heart tissue that combines image processing with deep learning for tissue detection, vessel demarcation, and fibrosis quantification. At the core of the pipeline is a mask-Convolutional Neural Network for segmentation of blood vessels. Cross-validation benchmarking of model predictions showed a median Dice Coefficient of 0.85 (on-par with expert performance) and a median pixel-wise prediction accuracy of 0.989, indicating robust prediction of blood vessel boundaries by the neural network. Time spent for inference of vessel boundaries per section by the model was 0.78s/image on a standard commercial machine, signifying the feasibility of running the pipeline on commercial hardware. We further show that inference times can be significantly reduced further using specialized hardware. As a “real-world” test of the pipeline, we analyzed fibrosis in trichrome-stained heart sections of healthy Wistar vs. Zucker Diabetic Fatty (ZDF) male rats. In concordance with established results, the pipeline was able to quantitatively show significantly higher inter-stitial fibrosis in ZDF rats, as compared to the Wistar controls.
...
* Make labelled images publicly available to support future work
* Algorithm is open source and modifiable, as opposed to Genie algorithm
* Genie needs new training for each dataset, which will generate variability from lab to lab

Similar tool that does gross (whole heart) quantification and does not show any classification specific metrics

Computational tools for automated histological image analysis and quantification in cardiac tissue. Daniel Gratza, b

Useful intro

Quantification of myocardial fibrosis by digital image analysis and interactive stereology

Interstitial fibrosis refs in intro

JavaCyte, a novel open‑source tool for automated quantification of key hallmarks of cardiac structural remodeling

Automated histopathology is more sensitive, faster, and auditable

Pros and cons of artiﬁcial intelligence implementation in diagnostic pathology

https://doi.org/10.1111/his.15153

# Introduction {#introduction}

Problem statement
Why it is important to solve the problem (disease burden, faster research, what is fibrosis associated with and what is the etiology of interstitial fibrosis)
Why is interstitial fibrosis a better metric than whole heart fibrosis calculation?
Current methodology to address the problem and where it falls short
How the method fills in the gaps and it's characteristics
...
![][image1]
**FIgure 1\. Overview of data structure and processing pipeline. A)** A collage of 12 slides from the same batch of tissues. Variability in tissue shape and placement highlights the challenge of accurate placement of regions-of-interest for fibrosis quantification. **B)**  Steps of image-processing. Left) Original stained slide, Middle) Slide after performing intensity thresholding, morphological operations, and labelling of tissues, Right) Placement of regions-of-interest on heart tissues, **C)** Mask-RCNN structure used for machine-labelling of blood-vessels. Adapted from \[REF\]
![][image2]
**Figure 2\. Properties of training dataset. A)** Distribution of labels per image. A single label may contain multiple blood vessels if the vessels were adjacent. Inset) Pie-chart showing fraction of images with and without labels. **B)** Total fraction of labelled area per image. **C)** Time taken to label images with different numbers of labels. **D)** Inter-rater reliability measured using Dice coefficient indicating expert-level similarity of labels (see text). Inset) Confusion matrix indicating similarity in labelling at the whole-section level. **E)** Representative images visualizing similarity in labels across 2 labellers. Labels for each labeller are indicated using different colored contours.

Similar inter-label reliability as radiologists:

* Wang, Yiren, et al. “Improved Automatic Segmentation of Brain Metastasis Gross Tumor Volume in Computed Tomography Images for Radiotherapy: A Position Attention Module for U-Net Architecture.” Quantitative Imaging in Medicine and Surgery, vol. 14, no. 7, Jul. 2024, pp. 4475489–4474489. qims.amegroups.org, [https://doi.org/10.21037/qims-23-1627](https://doi.org/10.21037/qims-23-1627)
* Wong, Jeffrey, et al. “Effects of Interobserver and Interdisciplinary Segmentation Variabilities on CT-Based Radiomics for Pancreatic Cancer.” Scientific Reports, vol. 11, Aug. 2021, p. 16328\. PubMed Central, [https://doi.org/10.1038/s41598-021-95152-x](https://doi.org/10.1038/s41598-021-95152-x).
* Covert, E.C., Fitzpatrick, K., Mikell, J. et al. Intra- and inter-operator variability in MRI-based manual segmentation of HCC lesions and its impact on dosimetry. EJNMMI Phys 9, 90 (2022). [https://doi.org/10.1186/s40658-022-00515-6](https://doi.org/10.1186/s40658-022-00515-6)
*

![][image3]
**Figure 3\.** **Neural network training. A)** Augmentation transformations used to increase robustness of network training. i) Original image, ii) horizontal flip, iii) vertical flip, iv) translation, v) rotation, vi) color perturbation, vii) removal of vessels, viii) multiplication of blood vessels. **B)** Multiple training runs of the network quantitatively converged to the same validation loss. **C)** Examples of prediction by the network. Top row: Original labelled data, Bottom row: Network predictions (green highlight marks area outside vessel)

![][image4]

## ![][image5]

**Figure 5\.** Comparison of whole-pipeline performance against user. **A)** Manual (user) and Auto (pipeline) labelled regions-of-interest on a representative tissue section. Heat of color in each ROI denotes interstitial fibrosis quantified. **B) and D)** Section- and tissue-level comparisons of manual and automated quantification show strong and statistically significant similarities between both methods of quantification. **C) and E)** Bland-Altman plots corresponding to B) and D) showing no significant bias in the comparison. **F)** Comparison of area covered by each method of quantification across 10 tissues. Inset) Automated method on-average covered \~3x more area than manual annotation. **G)** Comparison of time-taken per tissue section. Automated methodology took \~90% less total time, and \~95% less user time compared to manual methodology.
...
Reduction in vessel inference \+ region extraction time with increased resources
...
Hyperparameter optimization for future directions
...
* Quantifying myocyte pattern consistency
  * https://bmcbiomedeng.biomedcentral.com/articles/10.1186/s42490-019-0026-8https://bmcbiomedeng.biomedcentral.com/articles/10.1186/s42490-019-0026-8
* unsurprised extraction of visual features

* Comparison with UNet

* Feature extraction can be improved by incorporating unsupervised training on histological images as in Dosotivsky et al
